...
from adad.utils import (create_dir, drop_redundant_col, set_seed,
                        time2str, to_json)

CLASSIFIER_NAMES = ['rf', 'svm', 'knn', 'nn']
DESCRIPTORS = ['cddd', 'ecfp', 'rdkit']
...
def get_nn_params(X, clf_params):
    ...
    if clf_params['hidden_dim'] < clf_params['input_dim']:
        # Hidden layer cannot be smaller than input layer
        clf_params['hidden_dim'] = int(clf_params['input_dim'] * 2)

    return clf_params


def split_X_alternatives(X_alternatives, idx_train, idx_test):
    Xs_alt_train = []
    Xs_alt_test = []
    for x in X_alternatives:
        X_train, X_test = x[idx_train], x[idx_test]
        Xs_alt_train.append(X_train)
        Xs_alt_test.append(X_test)
    return Xs_alt_train, Xs_alt_test


def run_pipeline(X, X_alternatives, y, cv_train, cv_test, Classifier, clf_params,
    ...
    if Classifier.__name__ == NNClassifier.__name__:
        ...
        # Alternative descriptors also uses different parameters
        fs_clf_params = []
        for x_alt in X_alternatives:
            fs_clf_params.append(get_nn_params(x_alt, clf_params))
    else:
        # Classifiers for alternative descriptors use same parameter as the base one.
        fs_clf_params = [clf_params] * len(X_alternatives)
    ...
    for col in cv_train.columns:
        ...
        Xs_alt_tr, Xs_alt_te = split_X_alternatives(X_alternatives, idx_train, idx_test)
        assert len(X_train) == len(Xs_alt_tr[0])
        assert len(X_test) == len(Xs_alt_te[0])
        ...
        ad = SklearnFeatureSqueezing(
            clf,
            Classifier,
            n_discriptors=len(X_alternatives),
            clf_params=fs_clf_params,
        )
        ad.fit(Xs_alt_tr, y_train)

        dist_measure = ad.measure(X_test, Xs_alt_te)
...
def load_X_alternatives(path, dataname, desc=DESCRIPTORS):
    """Load alternative descriptors from Numpy arrays."""
    Xs = []
    for d in desc:
        path_data = os.path.join(path, d, f'{dataname}_{d}.npy')
        X = np.load(path_data)
        Xs.append(X)
    return Xs
...
def run_exp(path_input,
    ...
    path_outputs = os.path.join(path_output, f'{Classifier.__name__}_{SklearnFeatureSqueezing.__name__}')
    ...
    X_alternatives = load_X_alternatives(path_input, dataname)
    ...
    run_pipeline(X, X_alternatives, y,
                 cv_train, cv_test,
                 Classifier,
                 clf_params,
                 dataname,
